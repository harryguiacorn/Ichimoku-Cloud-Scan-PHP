...
<?php
	...
	if(DEBUG_MODE) echo "<h2>Update Started</h2>";
	$_cloudSignalMain = new CloudSignalMain("Cloud Signals","Forex");
	$_cloudSignalMain->init(CloudUtility::UPDATE_MODE_DAILY_FOREX_FROM_SCRATCH);
	
	$__timeFinish = microtime(true);
	$__timeSpent = round($__timeFinish - $__timeStart, 1);
	if(DEBUG_MODE) echo "<h2>Update took $__timeSpent seconds\n</h2>";
?>
</body>
</html>
